class Views::CashControls::DepositForm < Views::Base
  def initialize(form: nil)
    if form.present?
      @form = form
    else
      ctx = CashControl::Operations::Deposit::Present.call(params: form_params.to_h)
  ...
  def view_template
    div(class: "rounded-lg border p-6 bg-background") do
      div(class: "flex items-center gap-2 mb-4") do
        Remix::MoneyDollarBoxLine(class: "w-5 h-5 text-yellow-600")
        h3(class: "text-lg font-semibold text-gray-900 dark:text-white") { "Nhập quỹ tiền mặt" }
      end

      div(class: "space-y-4") do
        Form(action: form_url, method: "POST") do
          div(class: "deposit-wrapper") do
