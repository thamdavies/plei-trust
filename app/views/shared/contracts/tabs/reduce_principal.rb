    ...
    TabsContent(value: "reduce_principal") do
      div(class: "rounded-lg border p-6 space-y-4 bg-background text-foreground") do
        div(class: "space-y-0") do
          ...
          Table do
            TableCaption { "Lịch sử rút gốc sẽ được hiển thị ở đây" } if contract.active_reduce_principals.blank?
            TableHeader do
              TableRow do
                TableHead { "STT" }
            ...
            TableBody do
              contract.active_reduce_principals.includes(:transaction_type).each_with_index do |item, index|
                TableRow do
