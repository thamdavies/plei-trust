class Views::Incomes::Form < Views::Base
  def initialize(form:)
    @form = form
  end

  def view_template
    Form(action: incomes_path, method: :post, id: "income-form") do
      Input(type: "hidden", name: "authenticity_token", value: form_authenticity_token)
      div(class: "flex gap-2") do
        Remix::HandCoinLine(class: "w-6 h-6")
        h2(class: "text-md mb-2 font-medium text-gray-900 dark:text-white") { "Nhập phiếu thu tiền" }
      end
      FormField(class: "max-w-sm") do
        FormFieldLabel { "Người nộp tiền" }
      ...
      div(class: "flex gap-4 items-center") do
        FormField(class: "w-sm") do
          ...
          select(
            ...
            view_context.select_options_for_transaction_types(is_income: true).each do |item|
      ...
      Button(type: "submit", class: "mt-4") { "Thu tiền" }
    end
  end
