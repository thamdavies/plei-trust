class Views::Staffs::FilterForm < Views::Base
  def view_template
    div(class: "flex justify-between w-full") do
      div(class: "w-full") do
        div(class: "items-center mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0 dark:divide-gray-700") do
          Form(action: staffs_path, method: "GET", class: "sm:pr-3 space-y-6", data: { controller: "auto-submit" }) do |f|
            div(class: "flex items-center gap-4 mb-1") do
              ...
              FormField(class: "relative w-48 mt-1 sm:w-64 xl:w-96") do
                SearchInput(
                  name: "q[email_or_full_name_cont]",
                  placeholder: "Tìm kiếm tên, email...",
                  value: view_context.params.dig(:q, :email_or_full_name_cont)
                )
              end
      ...
      div(class: "items-center mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0 dark:divide-gray-700",
        data: { controller: "resource", resource_path_value: new_staff_path, resource_dialogbutton_value: "staff-dialog-trigger" }) do
